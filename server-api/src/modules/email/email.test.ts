...
describe('email module', () => {
  ...
  it('should delete multiple emails when authenticated', async () => {
    const app = createApp();
    const token = buildToken();
    ...
    const response = await request(app)
      .post('/api/email/delete-range')
      .set('Authorization', `Bearer ${token}`)
      .send({ ids: [1, 2] });

    expect(response.status).toBe(200);
    expect(response.body.status).toBe(true);
    expect(response.body.message).toBe('Emails have been deleted successfully.');
  });
});
